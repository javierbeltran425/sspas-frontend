...
export default function DinamicTableChart(props) {
    const { measurements = [], disaggregation1, disaggregation2, value1, value2 } = props;

    console.log(measurements);

    console.log('impresion de disagregation1');
    console.log(disaggregation1);
                ...
                for(let h = 0; h < measurements.length; h++){
                    if(measurements[h].ano === uniqueYearsArr[i]){
                        for(let j = 0; j < measurements[h].valores_factor.length; j++){
                            if(measurements[h].valores_factor[j].categoria === disaggregation1.nombre && measurements[h].valores_factor[j].valor === value1.name){
                                measurementsPerYears1[i] = measurementsPerYears1[i] + parseInt(measurements[h].valor_medicion)
                            }
                        }
                    }
                }
        ...
        if(uniqueYearsArr.length > 0){
            for(let i = 0; i < uniqueYearsArr.length; i++){
                for(let h = 0; h < measurements.length; h++){
                    if(measurements[h].ano === uniqueYearsArr[i]){
                        for(let j = 0; j < measurements[h].valores_factor.length; j++){
                            if(measurements[h].valores_factor[j].categoria === disaggregation2.nombre && measurements[h].valores_factor[j].valor === value2.name){
                                measurementsPerYears2[i] = measurementsPerYears2[i] + parseInt(measurements[h].valor_medicion)
                            }
                        }
                    }
                }
